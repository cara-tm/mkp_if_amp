...
* "Download":https://github.com/cara-tm/mkp_if_amp/blob/master/mkp_if_amp_v3.2_zip.txt
* "Demo Site for initial plugin":http://ampdemo.cmsstyles.com/

Don't forget to discover this other plugin Google AMP capable: "pat_article_social":https://github.com/cara-tm/pat_article_social

h2. mkp_if_amp

h3. Attributes (in v 3.3)

* @url@ _string_ (optional): The location (URL) of the website. Default: @hu@ (website URL set in the prefs panel).
* @subdomain@ _string_ (optional): Sets the subdomain name where to redirect. Default: @amp@.
* @permlink@ _boolean_ (optional): sets to @false@ ("0") the redirection for the same article's url mode within the subdomain will be not made. Default: @true@ ("1").
...
h2. mkp_amp_sanitize (in 3.5)

A tag to render article text contents without any inline styles. To use in replacement of the TXP @<txp:body />@ or @<txp:excerpt />@ tags.

h3. Attribute

* @content@ _string_ (optional): choose among "body" or "excerpt" what article content to sanitize. Default: _body_.

h3. Usage

Into an article form:

bc. <txp:mkp_amp_sanitize content="excerpt" />

h2. mkp_amp_redirect

A simple tag for web redirection to a subdomain name. Same feature in use within "The Guadian" website. Must be used with a mobiles detection script (i.e. adi_mobile plugin). <br>Some Web Designers could choose to use a subdomain for their mobiles websites (may be not recommanded. But the official AMP Blog is located into a subdomain, see: "https://amphtml.wordpress.com":https://amphtml.wordpress.com). In this case, the Textpattern multi-sites capacities makes things easier.
Note: Doesn't work on local server installation but only on real live servers.
...
Version 3.5: January 13th 2017. New tag added @<txp:mkp_amp_sanitize="body/excerpt" />@ for inline styles removing into article's text contents. Thks whocarez.
